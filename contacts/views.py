...
def sms_group_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        groups = ContactGroup.objects.all().order_by('name')
        data['html_sms_member_list'] = render_to_string('contacts/includes/partial_group_list.html', {
            'groups': groups
        })
    else:
        context = {'group': group}
...
def sms_member_create(request):
    if request.method == 'POST':
        ...
        form = SMSMemberForm()
    return save_form(request, form, 'contacts/includes/partial_contact_create.html')
...
def sms_member_update(request, pk):
    ...
    return save_form(request, form, 'contacts/includes/partial_contact_update.html')
